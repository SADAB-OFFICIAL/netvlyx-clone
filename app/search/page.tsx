...
function SearchPageContent() {
            ...
            {items.map((item, idx) => {
                ...
                return (
                    <motion.div 
                        ...
                        key={idx} 
                        className="group relative bg-white/5 rounded-2xl md:rounded-[2rem] overflow-hidden cursor-pointer border border-white/5 hover:border-yellow-500/50 transition-all duration-300 shadow-xl hover:shadow-[0_20px_40px_rgba(234,179,8,0.15)] hover:-translate-y-2 backdrop-blur-xl"
                    ...
                    >
                        <div className="aspect-[2/3] relative overflow-hidden bg-gray-900">
                            ...
                            <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                                <div className="w-14 h-14 bg-white/20 backdrop-blur-md border border-white/40 rounded-full flex items-center justify-center shadow-2xl scale-0 group-hover:scale-100 transition-transform duration-300 delay-75 hover:bg-yellow-500 hover:border-yellow-400">
                                    <Play className="text-white hover:text-black ml-1 transition-colors fill-current" size={24} />
                                </div>
                            </div>
                        </div>
                        <div className="p-4 md:p-5 relative z-10 bg-gradient-to-t from-black via-black/90 to-black/80">
                            <h3 className="text-gray-100 font-bold text-sm md:text-base leading-tight line-clamp-1 group-hover:text-yellow-400 transition-colors drop-shadow-md">{item.title}</h3>
                            <div className="flex items-center justify-between mt-3 text-xs text-gray-400 group-hover:text-gray-300 transition-colors">
                                ...
                                <span className="flex items-center gap-1 opacity-70"><Film size={12} /> {dynamicYear}</span>
                            </div>
                        </div>
                    </motion.div>
                );
  ...
  return (
    <div className="min-h-screen relative bg-[#050505] text-white font-sans selection:bg-yellow-500/30 overflow-x-hidden">
      ...
      {/* Floating Back Button (Always top left) */}
      <motion.button 
         initial={{ opacity: 0 }} animate={{ opacity: 1 }}
         onClick={() => { triggerHaptic('light'); router.push('/'); }} 
         className="absolute top-6 left-4 md:left-8 z-50 p-3 md:p-4 bg-white/5 backdrop-blur-xl border border-white/10 rounded-full text-gray-400 hover:text-white hover:bg-white/10 transition-all shadow-lg active:scale-95 cursor-pointer"
      >
          <ChevronLeft size={24} />
      </motion.button>

      {/* 🚀 MAC SPOTLIGHT / VISION PRO COMMAND CENTER 🚀 */}
      <motion.div 
         className="w-full max-w-4xl mx-auto px-4 relative z-40 flex flex-col items-center"
         // This is the Magic: Animates padding to slide between Center and Top
         animate={{ paddingTop: isActive ? "2rem" : "32vh" }}
         transition={{ type: "spring", stiffness: 200, damping: 25 }}
      >
          {/* Logo - Only shows when centered */}
          <AnimatePresence>
             {!isActive && (
                 <motion.div 
                 ...
                 >
                     <div className="w-20 h-20 bg-gradient-to-br from-yellow-400 to-amber-600 rounded-3xl flex items-center justify-center shadow-[0_0_50px_rgba(234,179,8,0.3)]">
                        <MonitorPlay className="text-black" size={40} />
                     </div>
          ...
          >
              ...
              <div className={`relative flex items-center bg-black/40 backdrop-blur-3xl border border-white/10 overflow-hidden shadow-[0_20px_60px_rgba(0,0,0,0.5)] transition-all duration-500 group-focus-within:border-yellow-500/50 group-focus-within:bg-black/60 ${isActive ? 'rounded-[2rem] p-3 md:p-4' : 'rounded-[3rem] p-4 md:p-6'}`}>
                  ...
                  {loading ? (
                     <Loader2 className="text-yellow-500 animate-spin ml-2 md:ml-4 shrink-0" size={isActive ? 24 : 32} />
                  ) : (
                     <Search className={`${isActive ? 'text-yellow-500' : 'text-gray-400'} ml-2 md:ml-4 shrink-0 transition-colors duration-500`} size={isActive ? 24 : 32} />
                  )}
                  ...
                  <input 
                     ...
                     className={`w-full bg-transparent border-none outline-none text-white ml-4 md:ml-6 placeholder-gray-500/70 font-medium transition-all duration-500 ${isActive ? 'text-lg md:text-xl' : 'text-2xl md:text-4xl'}`} 
                     autoFocus 
                  />
                  ...
                  <AnimatePresence>
                     {searchTerm && (
                         <motion.button 
                            ...
                            className="mr-2 md:mr-4 p-2 bg-white/10 hover:bg-white/20 rounded-full transition-colors cursor-pointer"
                         >
                            <X size={isActive ? 20 : 28} className="text-gray-300 hover:text-white" />
                         </motion.button>
                     )}
                  </AnimatePresence>
              </div>
          ...
          {/* Quick Filters (Shown only when active but no search term, or as pills below) */}
          <AnimatePresence>
            {!searchTerm && isActive && (
               <motion.div 
                 initial={{ opacity: 0, y: 20 }}
               ...
               >
                  {['Latest Movies', 'Trending Series', 'Netflix', 'Marvel', 'Anime'].map((tag) => (
                      <button 
                         ...
                         className="px-5 py-2.5 bg-white/5 hover:bg-white/15 border border-white/10 rounded-full text-sm font-medium text-gray-300 hover:text-white transition-all active:scale-95 cursor-pointer backdrop-blur-md"
                      >
                         {tag}
                      </button>
                  ))}
      ...
      {/* 🔮 RESULTS STAGE 🔮 */}
      <div className="relative z-30 w-full max-w-[1920px] mx-auto px-4 md:px-12 pb-24 mt-8 md:mt-16">
          <AnimatePresence>
              {isActive && searchTerm && (
                  <motion.div 
                  ...
                  >
                      ...
                      <div className="flex items-center gap-4 mb-8 border-b border-white/10 pb-6">
                          <h1 className="text-2xl md:text-3xl font-black text-white flex items-center gap-3 tracking-tight">
                              {loading ? (
                                  <span className="flex items-center gap-3 text-yellow-500">
                      ...
                      {/* SECTION 1: MOVIESDRIVE (Cloud Server 1) */}
                      ...
                      {/* SECTION 2: MOVIES4U (Cloud Server 2) */}
                      {server2Results.length > 0 && (
                          <>
                             {server1Results.length > 0 && <div className="h-px bg-gradient-to-r from-transparent via-white/20 to-transparent my-12"></div>}
                      ...
                      {!loading && searchTerm && results.length === 0 && (
                          ...
                          >
                              ...
                              <div>
                                 <h2 className="text-2xl md:text-3xl font-bold text-white mb-2">Universe Scanned</h2>
                                 <p className="text-gray-500 max-w-md">We couldn't find anything matching your search. Try different keywords or check for typos.</p>
                              </div>
                          </motion.div>
                      )}
                  </motion.div>
