...
export default function MoviePage() {
  ...
  // 2. Fetch TMDB Data (With Year Logic)
  useEffect(() => {
    if (!data) return;
    ...
    if (data.imdbId) {
        ...
        if (data.year) apiUrl += `&year=${data.year}`; // ✅ Sending Year
    }
      ...
      <div className="relative w-full h-[80vh] md:h-[90vh]">
          ...
          <div className="relative z-10 flex flex-col items-center justify-end h-full pb-16 px-4 text-center max-w-4xl mx-auto animate-slide-up">
              ...
              <div className="flex gap-4">
                  <button onClick={() => handleHeroAction('watch')} className="bg-white text-black px-8 py-3.5 rounded-full font-bold flex items-center gap-2 hover:scale-105 transition-transform"><Play className="fill-current" size={20}/> Watch Now</button>
                  <button onClick={() => handleHeroAction('download')} className="bg-gray-800/60 backdrop-blur-md text-white px-8 py-3.5 rounded-full font-bold flex items-center gap-2 border border-white/20 hover:bg-gray-800 transition-colors"><Download size={20}/> Download</button>
              </div>
          </div>
      </div>
                  ...
                  {availableSeasons.length > 0 && selectedSeason === null && (
                      <div className="animate-fade-in">
                          ...
                          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3">
                              {availableSeasons.map(s => <button key={s} onClick={() => setSelectedSeason(s)} className="p-4 bg-gray-800 hover:bg-red-600 border border-gray-700 rounded-xl font-bold text-lg transition-all">Season {s}</button>)}
                          </div>
                      </div>
                  )}
