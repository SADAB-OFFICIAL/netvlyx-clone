...
import { 
  ...
  Copy, CheckCircle, Wifi, HardDrive, MonitorPlay, 
  Lock, ExternalLink, X 
} from 'lucide-react';
...
function NCloudPlayer() {
      ...
      const init = async () => {
        try {
          ...
          if (ncloudData.mode === 'direct') {
              // ✅ V-CLOUD MODE
              if (!ncloudData.streams?.length) throw new Error("No streams found from V-Cloud source");
              ...
              if (!genData.success || (!genData.streams?.length && !genData.streamLink)) {
                  throw new Error("Final link generation failed");
              }

              if (genData.streams && genData.streams.length > 0) {
                  finalStreams = genData.streams;
              } else {
                  finalStreams = [{ server: "⚡ Fast Cloud (VIP)", link: genData.streamLink, type: "DIRECT" }];
              }
              finalTitle = genData.filename;
          }

          setStreams(finalStreams);
          setCurrentStream(finalStreams[0]);
  ...
  const openManualLink = () => {
      if (metaData?.url || metaData?.link) {
          window.open(metaData.url || metaData.link, '_blank');
      }
  };
  ...
  if (loading) return (
    <div className="min-h-screen bg-[#050505] flex flex-col items-center justify-center relative overflow-hidden px-4">
      ...
      <div className="relative z-10 flex flex-col items-center text-center">
         ...
         <h2 className="text-xl md:text-2xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-purple-400 animate-pulse mb-2">Secure Connection...</h2>
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-white font-sans overflow-x-hidden relative pb-10">
      ...
      {/* 🔴 ERROR POPUP (Modal) */}
      {(showErrorPopup || (!currentStream && !loading)) && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-fade-in">
              <div className="bg-[#121212] border border-white/10 rounded-2xl w-full max-w-sm p-6 relative shadow-2xl">
                  {/* Close Button */}
                  <button 
                  ...
                  <div className="text-center">
                      ...
                      <p className="text-gray-400 text-sm mb-6">
                          {statusMsg !== 'Initializing Secure Handshake...' ? statusMsg : "We couldn't generate a direct link automatically."}
                      </p>
                      ...
                      <div className="space-y-3">
                          ...
                          >
                              <ExternalLink size={18} /> Open Manual Link
                          </button>
            ...
            <div className="flex items-center gap-3 w-full md:w-auto">
                ...
                <div>
                    ...
                    <div className="flex items-center gap-2 text-[10px] md:text-xs font-mono text-gray-400">
                        ...
                        {currentStream ? `ENCRYPTED • ${currentStream.server}` : 'OFFLINE'}
                    </div>
                </div>
            </div>
